...
class RealtimeGraphServer:
    ...
    async def _handle_http_index(self, writer: asyncio.StreamWriter) -> None:
        ...
        if not index_path.exists():
            await self._send_http_response(writer, 404, "Not Found", "text/plain", b"index.html not found")
            return
        
        try:
            with open(index_path, 'rb') as f:
                content = f.read()
            await self._send_http_response(writer, 200, "OK", "text/html", content)
        except Exception:
            await self._send_http_response(writer, 500, "Internal Server Error", "text/plain", b"Error reading file")
    ...
    async def _handle_ws_upgrade(
        ...
        # Handle incoming messages with proper timeout handling
        try:
            while not ws.closed:
                try:
